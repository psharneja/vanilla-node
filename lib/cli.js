...
cli.responders.listLogs = function () {
    ...
    // var ls = childProcess.spawn('atom', ['./']);

    // var ls = childProcess.spawn('ls', ['./.logs/']);
    // ls.stdout.on('data', function (dataObject) {
    //     //explode into separate lines
    //     var dataString = dataObject.toString();
    //     var logFileNames = dataString.split('\n');
    //     cli.verticalSpace();
    //     logFileNames.forEach(function (logFileName) {
    //         if (typeof logFileName == 'string' && logFileName.length > 10 && logFileName.indexOf('-') > -1) {
