...
handlers._tokens.post = function (data, callback) {
    ...
    _performance.mark('inputs validation');
    ...
    if (phone && password) {
        ...
        _data.read("users", phone, function (err, userData) {
            ...
            if (!err && userData) {
                ...
                var hashedPassword = helpers.hash(password);
                _performance.mark('completed password hash');

                if (hashedPassword == userData.hashedPassword) {
                    // if valid, creata a new token with a random name, set expiration date 1 hour future
                    _performance.mark('beginning data creation for token');
                    ...
                    // store the token
                    _performance.mark('beginning token store');
                    ...
                    _data.create("tokens", tokenId, tokenObject, function (err) {
                        ...
                        _performance.measure('validation user input', 'entered function', 'inputs validation')
                        _performance.measure('vuser lookup', 'beginning user lookup', 'beginning user lookup')
                        _performance.measure('password hashing', 'beginning password hash', 'completed password hash')
                        _performance.measure('token data creation', 'beginning data creation for token', 'beginning token store')
                        _performance.measure('token storinng', 'beginning token store', 'storing token complete')

                        //logout all measurement
                        //not in node10
                        // var measurements = _performance.getEntries('measure');
                ...
                _data.read("users", userPhone, function (err, userData) {
                    if (!err && userData) {
                        ...
                        if (userChecks.length < config.maxChecks) {
                            //verify that the url given has dns entries and can be resolved
                            ...
                            dns.resolve(hostName, function (err, records) {
                                if (!err && records) {
                                    ...
                                    callback(400, { 'Error': 'Hostname url dns doesnt exist' })
                                }
                            })
